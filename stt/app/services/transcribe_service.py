...
class TranscribeService:
    ...
    async def transcribe_audio(self, session_id: str) -> User:
        ...
        logger.info(f"Reading audio file: container={self.store_service.container_name}, file={user.session.audio.name}")
        audio_data = compress_audio_file(
            await self.store_service.get_file(session_id, user.session.audio.name, binary=True)
        )
